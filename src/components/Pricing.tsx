  ...
  return (
    ...
    >
      <motion.div
        ...
        viewport={{ once: true }}
        variants={fadeUp}
        className="mb-16 grid lg:grid-cols-2 gap-12 items-end"
      >
        <div>
          <div
        ...
        {pricing.map((t, i) => {
          ...
          return (
            ...
            >
              {t.recommended && (
                <span
                  className="absolute -top-3 left-5 px-2 py-1 text-[10px] uppercase tracking-[0.12em]"
              ...
              <div
                className="font-head text-[42px] font-semibold mb-6 leading-none"
              ...
              <ul className="space-y-3 mb-8 flex-1">
                {t.features.map((f) => (
                  <li key={f} className="flex items-start gap-3 text-[13px]">
                    <span
                      className="mt-[5px] w-2.5 h-2.5 flex-shrink-0"
              ...
              <a
                href={`/#contact?service=${encodeURIComponent(t.service)}&price=${encodeURIComponent(String(t.price))}`}
              ...
              >
                <RollingButtonText text={`Select ${t.name} \u2192`} />
              </a>
            </motion.div>
          );
        })}
